...
template <typename T>
inline void *GetOptionalDataPtr(comm_gemm_overlap::optional_tensor_ref x, int64_t index) {  // NOLINT
...
void ub_paddle_allgather(void *globaldata, size_t globalbytes, void *localdata, size_t localbytes,
  ...
  const auto localtensor =
    paddle::from_blob(
      localdata, 
      {static_cast<int64_t>(localbytes / sizeof(uint8_t))},
  ...
  auto globaltensor =
    paddle::from_blob(
      globaldata, 
      {static_cast<int64_t>(globalbytes / sizeof(uint8_t))},
      paddle::DataType::UINT8, 
      phi::DataLayout::NCHW, //default layout
      phi::CPUPlace()
  );
...
UbufP2PCommOverlap::UbufP2PCommOverlap(
  ...
  _ubuf_dtype = (element_size == 1) ? DType::kFloat8E4M3
  ...
  for (int i = 0; i < _num_ubuf_chunks; i++) {
    auto ubuf_chunk = paddle::from_blob(
        ubuf_byte_ptr, kChunkShape, sample.dtype(), sample.layout(), sample.place());
...
void UbufP2PCommOverlap::split_overlap_ag(
  ...
  assert(!_ubufs.empty());
  const std::initializer_list<size_t> kChunkShape{static_cast<size_t>(_ubufs[0].shape()[0]), static_cast<size_t>(_ubufs[0].shape()[1])};  
  std::vector<TensorWrapper> ubufs_;
  ubufs_.reserve(_num_ubuf_chunks);  
  for (int i = 0; i < _num_ubuf_chunks; i++) {
    ubufs_.emplace_back( //i.e. MakeNvteTensor
        _ubufs[i].data(), kChunkShape, _ubuf_dtype, 
        nullptr, nullptr, _ubuf_scale_inv_ptr);
  }

  const auto stream_main = A.stream(); //at::cuda::CUDAStream stream_main = at::cuda::getCurrentCUDAStream();
  CommGemmOverlapP2P::split_gemm_overlap_ag(stream_main, A_, transa, B_, transb,
...
void UbufP2PCommOverlap::split_overlap_rs(
  ...
  for (int i = 0; i < _num_ubuf_chunks; i++) {
    ubufs_.emplace_back( //i.e. MakeNvteTensor
        _ubufs[i].data(), kChunkShape, _ubuf_dtype, 
        D_.amax(), D_.scale(), _ubuf_scale_inv_ptr);
  }

  const auto stream_main = A.stream(); //at::cuda::CUDAStream stream_main = at::cuda::getCurrentCUDAStream();
  transformer_engine::common::CommGemmOverlapP2P::split_gemm_overlap_rs(stream_main, A_, transa, B_, transb,
...
void UbufP2PCommOverlap::copy_input_to_ubuf(const paddle::Tensor &input, bool chunk) {
  ...
  NVTE_CHECK_CUDA(cudaMemcpyAsync(ubuf.data(), input.data(),
                                  input.numel() * get_element_size(input),
...
std::tuple<TensorWrapper/*A_*/, 
  ...
  const DType gelu_dtype = pre_gelu_out ? Paddle2NvteDType(pre_gelu_out->dtype()) : Int2NvteDType(D_type);
  auto pre_gelu_out_ =
